...
export const ChatPage: React.FC = () => {
  ...
  const startCall = async (audioOnly = false) => {
    ...
    try {
      ...
      // Attempt to strictly schedule a recorded Meeting. If test database is empty this will fail.
      const res = await meetingsApi.createMeeting({
        title: `Call with ${chatPartner?.name}`,
        description: 'Instant Chat Call',
        participants: [userId],
        startTime: new Date().toISOString(),
        endTime: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
        meetingType: audioOnly ? 'audio' : 'video'
      });
      setActiveMeetingId(res.data.data.meeting._id);
    } catch (err) {
      console.warn('DB meeting creation failed due to mock users. Launching ad-hoc WebRTC session anyway.');
      // Backend handles unverified meeting strings elegantly as Ad-hoc rooms!
      const adhocRoom = `adhoc_${currentUser.id}_${userId}`;
      setActiveMeetingId(adhocRoom);
    }
  };
